...
function getData() {
    ...
    $.ajax({
        url: "../../plmis_src/api/get-reporting-rate.php",
        type: "GET",
        data: {
            year: year,
            month: month,
...
function cLMISLayer(wkt, province, product, StkHolder, district_id, district_name, reported, total_warehouse, reporting_rate) {
    ...
    if (reporting_rate < classesArray[0].end_value) {
    ...
    if (reporting_rate > classesArray[6].start_value && reporting_rate <= classesArray[6].end_value) {
