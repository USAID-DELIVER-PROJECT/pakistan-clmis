$(window).load(function() {
    ...
    province = new OpenLayers.Layer.Vector(
        "Province", {
            protocol: new OpenLayers.Protocol.HTTP({
                url: "../../plmis_js/maps/province.geojson",
    ...
    district = new OpenLayers.Layer.Vector(
        "District", {
            protocol: new OpenLayers.Protocol.HTTP({
                url: "../../plmis_js/maps/district.geojson",
                format: new OpenLayers.Format.GeoJSON({
                    internalProjection: new OpenLayers.Projection("EPSG:3857"),
                    externalProjection: new OpenLayers.Projection("EPSG:3857")
                })
...
function getData() {
    ...
    $.ajax({
        url: "../../plmis_src/api/get-cyp-pop-generated.php",
        type: "GET",
        data: {
            datefrom: date_from,
            dateto: date_to,
...
function cLMISLayer(wkt, province_id, province, product, StkHolder, district_id, district, cyp, pop, cyp_pop) {
    ...
    if (cyp_pop < classesArray[0].end_value) {
        color = classesArray[0].color_code;
        DataProblem = Number(DataProblem) + 1;
    ...
    if (cyp_pop > classesArray[5].start_value && cyp_pop <= classesArray[5].end_value) {
        color = classesArray[5].color_code;
        class4 = Number(class4) + 1;
        status = classesArray[5].description;
    }
    if (cyp_pop > classesArray[6].start_value) {
        color = classesArray[6].color_code;
        class5 = Number(class5) + 1;
        status = classesArray[6].description;
    }
...
function cLMISMiniLayer(wkt, province_id, province, product, StkHolder, district_id, district, cyp, pop, cyp_pop) {
    feature = new OpenLayers.Feature.Vector(wkt);
    if (cyp_pop < parseInt(classesArray[0].end_value)) {
        color = classesArray[0].color_code;
        DataProblem = Number(DataProblem) + 1;
    } else if (cyp_pop == parseInt(classesArray[1].start_value) && cyp_pop == parseInt(classesArray[1].end_value)) {
        color = classesArray[1].color_code;
        NoData = Number(NoData) + 1;
    } else {
        color = classesArray[2].color_code;
    ...
    feature.attributes = {
        ...
        cyp: cyp.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
        pop: pop.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
        value: cyp_pop.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
        color: color
    };
    cLMIS.addFeatures(feature);
    $("#loader").css("display", "none");
...
function districtCountGraph() {
    ...
    var DP = CalculatePercent(DataProblem, maximum);
  ...
  if (min == max) {
        ...
        pieArray.push({
            label: classesArray[6].description,
            value: cls5,
            color: classesArray[6].color_code
        });
    }
