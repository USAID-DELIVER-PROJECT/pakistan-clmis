$(function(){
	...
	{
		var req_no,recip,refrenceno,issuedBy,whTo;
		req_no=$("#issue_no").val();
		recip=$("#recipient").val();
		refrenceno=$("#issue_ref").val();
		whTo=$("#whTo").val();
		issuedBy=$( "#issued_by option:selected" ).text();;
		window.open('stock_issue_voucher.php?req_no='+req_no+'&recip='+recip+'&refrenceno='+refrenceno+'&issuedBy='+issuedBy+'&whTo='+whTo,'_blank','width=842,height=595');
	}
	);
