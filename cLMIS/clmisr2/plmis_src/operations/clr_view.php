...
if (isset($_REQUEST['id']) && isset($_REQUEST['wh_id'])) {
    ...
    while ($row = mysql_fetch_array($qryRes)) {
		...
		{
			$avgConsumption[$row['itmrec_id']] = '';
			$SOHDist[$row['itmrec_id']] = '';
			$SOHField[$row['itmrec_id']] = '';
			$totalStock[$row['itmrec_id']] = '';
			$desiredStock[$row['itmrec_id']] = '';
			$replenishment[$row['itmrec_id']] = '';
		}
		else
		{
			$avgConsumption[$row['itmrec_id']] = number_format($row['avg_consumption']);
			$SOHDist[$row['itmrec_id']] = number_format($row['soh_dist']);
			$SOHField[$row['itmrec_id']] = number_format($row['soh_field']);
			$totalStock[$row['itmrec_id']] = number_format($row['total_stock']);
			$desiredStock[$row['itmrec_id']] = number_format($row['desired_stock']);
			$replenishment[$row['itmrec_id']] = number_format($row['replenishment']);
		}
                            ...
                            <div id="printing" style="clear:both;margin-top:20px;">
                                <div style="margin-left:0px !important; width:100% !important;">
                                    ...
                                    <table width="100%">
                                        ...
                                        <tr id="doNotPrint">
                                            <td colspan="4" style="text-align:right; border:none; padding-top:15px;"><input type="button" onClick="printContents()" value="Print" class="btn btn-warning"/>
                                                <input type="button" onClick="history.go(-1)" value="Back" class="btn btn-warning"/></td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
