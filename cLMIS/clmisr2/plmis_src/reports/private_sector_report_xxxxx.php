...
WHERE stakeholder.lvl=4 and tbl_wh_data.item_id = '".$itemName."' AND tbl_wh_data.report_year = '".$_POST['year_sel']."' $where AND tbl_wh_data.report_month = '".$i."' GROUP BY tbl_wh_data.report_month ";
		...
		$querymaxyear = "SELECT
							DATE_FORMAT(MAX(tbl_wh_data.RptDate), '%Y')
						FROM
							tbl_wh_data
		...
		while ($row = mysql_fetch_array($proNamesQry)){
			...
			$xmlstore .="\t\t<cell>$row[itm_name]</cell>\n";
			for ($i=1; $i<=12; $i++){
			$qry="SELECT sum(tbl_wh_data.fld_issue_up) AS total FROM tbl_warehouse JOIN stakeholder ON stakeholder.stkid = tbl_warehouse.stkid INNER JOIN tbl_wh_data ON tbl_warehouse.wh_id = tbl_wh_data.wh_id WHERE tbl_wh_data.item_id = '".$itemName."' AND tbl_wh_data.report_year = '".$sel_year."' AND stakeholder.stkid IN (SELECT stkid FROM stakeholder WHERE stk_type_id = 1) AND tbl_wh_data.report_month = '".$i."' GROUP BY tbl_wh_data.report_month ";
