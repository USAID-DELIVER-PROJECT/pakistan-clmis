...
$report_title = "Non-reported Field Stores Report for";
$actionpage = "non_report_fields.php";
$parameters = "TSP";
...
$in_item =  'IT-001';
?>
<?php startHtml($system_title." - Non-Reported Warehouses Report");?>
...
<SCRIPT LANGUAGE="JAVASCRIPT" TYPE="TEXT/JAVASCRIPT">
    ...
    $query = "SELECT COUNT(distinct tbl_warehouse.wh_id) 
              FROM tbl_warehouse
              WHERE 1 $whtypestr $qstrstk $qstrprov";
...
where $lvlType AND ($obla <> 0 OR $recd OR $issued OR $adja OR $adjb OR $cbla <> 0 ) $qstrstk $qstrprov
               AND ( tbl_wh_data.report_month = ".$sel_month." AND tbl_wh_data.report_year = ".$sel_year.") $whtypestr";
			  // print $query2;
...
Inner Join stakeholder ON tbl_warehouse.stkofficeid = stakeholder.stkid
where  $lvlType AND 
				   ($obla <> 0 OR $recd OR $issued OR $adja OR $adjb OR $cbla <> 0  ) AND 
				   
				   ( tbl_wh_data.report_month = ".$sel_month." AND tbl_wh_data.report_year = ".$sel_year.") $qstrstk $qstrprov $whtypestr";
	//(($obla +$recd +$issued +$adja +$adjb + $cbla) = 0 ) AND
...
//					  LEFT OUTER JOIN tbl_districts ON tbl_districts.whrec_id = tbl_warehouse.dist_id
		...
		$queryr2 = "SELECT DISTINCT(tbl_warehouse.wh_id) 
					  FROM tbl_warehouse
					  WHERE 1 $whtypestr $qstrstk $qstrprov $qwhlist ORDER BY prov_id ASC";
		...
		$rowcolor = '#dff2a9';
			...
			while($rowr2 = mysql_fetch_array($rsr2)) {
				...
				$querywh = "SELECT
								...
								Inner Join stakeholder ON tbl_warehouse.stkofficeid = stakeholder.stkid
							WHERE wh_id=".$rowr2['wh_id']." ORDER BY province.LocName ,Districts.LocName";
...
//////////// GET Province/Region Name
	...
	if ($sel_prov == 'all' || $sel_prov == ""){
		...
		$provinceQryRes = mysql_fetch_array(mysql_query("SELECT LocName AS prov_tittle FROM tbl_locations WHERE PkLocID = '".$sel_prov."' "));
...
<script>
	...
	function doInitGrid(){
		...
		mygrid.setHeader("<div style='text-align:center; font-size:14px; font-weight:bold; font-family:Helvetica'><?php echo "Non-reported Warehouses Report For Stakeholder = $stkName And Province/Region = $provinceName (".date('F',mktime(0,0,0,$sel_month)).' '.$sel_year.")";?></div>,#cspan,#cspan,#cspan,#cspan");
		mygrid.attachHeader("<span title='Serial Number'>SNO</span>,<span title='Province/Region Name'>Province/Region</span>,<span title='District Name'>District</span>,<span title='Stakeholder Name'>Stakeholder</span>,<span title='Warehouse Type'>WH Type</span>,<span title='TelePhone Number'>TelePhone No</span>");
		mygrid.attachHeader(",#select_filter,#select_filter,#select_filter,#select_filter");
		mygrid.setInitWidths("100,150,150,160,*,160");
