	...
	$xmlstore .="<rows>\n";
		...
		while ($row = mysql_fetch_array($proNamesQry)){
			...
			if ($_POST['repIndicators'] == 1){
				...
				for ($i=1; $i<=12; $i++){
					$getTotal = mysql_query("SELECT sum(tbl_wh_data.wh_received) AS total FROM tbl_warehouse JOIN stakeholder ON stakeholder.stkid = tbl_warehouse.stkofficeid INNER JOIN tbl_wh_data ON tbl_warehouse.wh_id = tbl_wh_data.wh_id WHERE stakeholder.lvl > 3 AND tbl_wh_data.item_id = '".$itemName."' AND tbl_wh_data.report_year = '".$_POST['year_sel']."' $where AND tbl_wh_data.report_month = '".$i."' GROUP BY tbl_wh_data.report_month ");
					$row1 = mysql_fetch_array($getTotal);
					$monthSum = number_format($row1['total']);
					$xmlstore .="\t\t<cell>$monthSum</cell>\n";
				}
                    ...
                    <tr>
                        <td bgcolor="#FFFFFF" colspan="2">
                            ...
                            <table cellspacing="3" cellpadding="3" border="0">
                                <tbody>
                                    <tr bgcolor="#FFFFFF">
                                        <td bgcolor="#FFFFFF" class="sb1NormalFont" width="70"><strong>Filter by</strong></td>
                                        <td bgcolor="#FFFFFF">
                                        ...
                                        <!-- Stakeholder -->
                                        <td bgcolor="#FFFFFF">
											<span class="sb1NormalFont">Stakeholder</span>
                                        <?php 
                                        ...
                                        <td bgcolor="#FFFFFF">
											<span class="sb1NormalFont">Indicator</span>
                                        ...
                                        <td bgcolor="#FFFFFF"><input type="submit" class="input_button" value="GO" id="go" name="go" style="margin-top:15px;"></td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
