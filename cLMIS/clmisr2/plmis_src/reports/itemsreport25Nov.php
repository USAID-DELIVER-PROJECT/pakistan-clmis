...
$queryItem = " SELECT DISTINCT tbl_warehouse.wh_id,tbl_warehouse.wh_name as firstCol, tbl_warehouse.stkid as secondCol,tbl_warehouse.prov_id,stakeholder.stkname
			...
			WHERE Office.lvl = 1  AND stakeholder.stk_type_id = 0 and tbl_wh_data.report_month='$sel_month' and tbl_wh_data.report_year='$sel_year' and tbl_warehouse.prov_id=10
			 $qStrPro $qStrStk ";
$rsItem = mysql_query($queryItem) or die(mysql_error());
...
while($rowItem = mysql_fetch_array($rsItem)) 
{
 ...
 $xmlstore2 .="</rows>\n";
				...
				$queryItem = "SELECT DISTINCT Office.stkname AS firstCol,
										...
										INNER JOIN itminfo_tab ON stakeholder_item.stk_item = itminfo_tab.itm_id
										INNER JOIN tbl_wh_data ON tbl_warehouse.wh_id = tbl_wh_data.wh_id 
									WHERE Office.lvl = 2  AND stakeholder.stk_type_id = 0 and tbl_wh_data.report_month='$sel_month' and tbl_wh_data.report_year='$sel_year'
				  ...
				  if($numItem2>0) { 
					...
					$xmlstore1 .="\t<row id=\"$counter\">\n";				   
					  ...
					  $query = mysql_query("SELECT REPgetConsumptionAVG('WSPD','$sel_month','$sel_year','".$sel_item."','".$rowItem2['stkid']."','".$rowItem2['prov_id']."','".$rowItem2['dist_id']."') FROM DUAL");
