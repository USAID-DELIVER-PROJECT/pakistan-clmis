...
//if (mysql_num_rows ($rsStk3)>0)
		 ...
		 {
			 $counter++;
			//$queryvals2 =  "SELECT REPgetData('CABM','R','TSPD','$sel_month','$sel_year','".$sel_item."','".$sel_stk."','$sel_prov','".$rowdist['whrec_id']."') AS Value FROM DUAL";
			
			if($sel_stk == 'all' && $rptType == 'all')
			{
				$queryvals2 = "SELECT REPgetData('CABMY','R','TXPD','$sel_month','$sel_year','".$sel_item."','".$sel_stk."','$sel_prov','".$rowdist['whrec_id']."') AS Value FROM DUAL";
			}
			else if($sel_stk == 'all' && $rptType == 'public')
			{
				$queryvals2 = "SELECT REPgetData('CABMY','R','TPD','$sel_month','$sel_year','".$sel_item."','".$sel_stk."','$sel_prov','".$rowdist['whrec_id']."') AS Value FROM DUAL";
			}
			else if($sel_stk == 'all' && $rptType == 'private')
			{
				$queryvals2 = "SELECT REPgetData('CABMY','R','XPD','$sel_month','$sel_year','".$sel_item."','".$sel_stk."','$sel_prov','".$rowdist['whrec_id']."') AS Value FROM DUAL";
			}
			else if($sel_stk != 'all' && $rptType == 'public')
			{
				$queryvals2 = "SELECT REPgetData('CABMY','R','TSPD','$sel_month','$sel_year','".$sel_item."','".$proStkID."','$sel_prov','".$rowdist['whrec_id']."') AS Value FROM DUAL";
			}
			else if($sel_stk != 'all' && $rptType == 'private')
			{
				$queryvals2 = "SELECT REPgetData('CABMY','R','XPD','$sel_month','$sel_year','".$sel_item."','".$rowStk3['stkid']."','$sel_prov','".$rowdist['whrec_id']."') AS Value FROM DUAL";
			}
			else if($sel_stk != 'all' && $rptType == 'all')
			{
				$queryvals2 = "SELECT REPgetData('CABMY','R','TSPD','$sel_month','$sel_year','".$sel_item."','".$sel_stk."','$sel_prov','".$rowdist['whrec_id']."') AS Value FROM DUAL";
			}
			...
			$xmlstore .="\t\t<cell><![CDATA[$rowdist[wh_name]]]></cell>\n";
...
<script>
	...
	function doInitGrid(){
		...
		mygrid.setHeader("<div style='text-align:center; font-size:14px; font-weight:bold; font-family:Helvetica'><?php echo "District Report For Sector = '".ucwords($rptType)."' Stakeholder(s) = $stkName Province/Region = $provinceName  And Product = $prodName (".date('F',mktime(0,0,0,$sel_month)).' '.$sel_year.")";?></div>,#cspan,#cspan,#cspan,#cspan,#cspan");
		mygrid.attachHeader("<span title='District Name'>Districts</span>,<span title='Product Consumption'>Consumption</span>,<span title='Average Monthly Consumption'>AMC</span>,<span title='Product On Hand'>On Hand</span>,<span title='Month of Scale'>MOS</span>,#cspan");
