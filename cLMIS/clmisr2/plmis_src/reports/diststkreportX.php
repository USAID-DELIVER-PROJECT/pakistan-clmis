<?php
echo "This report is temporary unavailable (Max till 6 PM 16th April 13) ";
exit;
	ob_start();
	/***********************************************************************************************************
...
<?php
	...
	if($sel_prov=='all')
	{
		$querydist = "SELECT distinct tbl_locations.PkLocID AS whrec_id, tbl_locations.LocName AS wh_name, tbl_locations.ParentID as province
						FROM  tbl_locations
						INNER JOIN tbl_warehouse ON tbl_warehouse.locid = tbl_locations.PkLocID
						WHERE tbl_locations.LocLvl = 3
	...
	else
	{
		$querydist = "SELECT distinct tbl_locations.PkLocID AS whrec_id, tbl_locations.LocName AS wh_name, tbl_locations.ParentID as province
						FROM tbl_locations
						INNER JOIN tbl_warehouse ON tbl_warehouse.locid = tbl_locations.PkLocID
						 WHERE tbl_locations.LocLvl = 3 AND tbl_locations.ParentID = '$sel_prov' ORDER BY wh_name ASC";
	}
	/*print $querydist;
	exit;*/
	...
	while($rowdist = mysql_fetch_array($rsdist)) {
		...
		$queryStk = "SELECT DISTINCT(stkname), stakeholder.stkid
						FROM stakeholder
						INNER JOIN tbl_warehouse ON tbl_warehouse.stkid = stakeholder.stkid
						WHERE tbl_warehouse.prov_id=".$rowdist['province']." ".$where." ORDER BY stakeholder.stkid ASC";
		//print $queryStk;				// 
		 ...
		 while($rowStk3 = mysql_fetch_array($rsStk3)) {
			 ...
			 $queryvals2 =  "SELECT REPgetData('CABMY','R','WSPD','$sel_month','$sel_year','".$sel_item."','".$rowStk3['stkid']."','$sel_prov','".$rowdist['whrec_id']."') AS Value FROM DUAL";
