	...
	function getTotalCBLA($sel_prod,$sel_month,$sel_year,$sel_stk){
		...
		$queryItem = "SELECT tbl_wh_data.wh_cbl_a as thirdCol 
							  ...
							  WHERE 
								...
								tbl_warehouse.wh_type_id = 'CWH' $sel_stk";
		...
		$queryItem1 = "SELECT tbl_wh_data.wh_cbl_a as thirdCol 
							  ...
							  WHERE 
								...
								report_year='$sel_year' AND 
								item_id='$sel_prod' AND
								tbl_warehouse.wh_type_id = 'PPIU' $sel_stk";
		...
		$queryItem2 = "SELECT wh_cbl_a,wh_cbl_a
									...
									WHERE tbl_warehouse.wh_type_id IN ('DPWO','EDO(H)','DPIU','Private Sector') AND
			...
			return false;
				...
				<?php
				  ...
				  $queryItem = "SELECT tbl_warehouse.wh_type_id as firstCol,
							  ...
							  WHERE 
								report_month='$sel_month' AND 
								report_year='$sel_year' AND 
								tbl_warehouse.wh_type_id = 'CWH' $qStrPro $qStrStk $qStrProv GROUP BY tbl_warehouse.wh_id ";
				  $rsItem = mysql_query($queryItem) or die(mysql_error());
				...
				$queryItem = "SELECT tbl_warehouse.wh_type_id as firstCol,
							  ...
							  WHERE 
								report_month='$sel_month' AND 
								report_year='$sel_year' AND 
								tbl_warehouse.wh_type_id = 'PPIU' $qStrPro $qStrStk $qStrProv";
				  $rsItem = mysql_query($queryItem) or die(mysql_error());
                  ...
                  $queryItem2 = "SELECT wh_cbl_a,wh_cbl_a,tbl_warehouse.dist_id,tbl_warehouse.stkid,tbl_warehouse.prov_id
									FROM tbl_wh_data
									LEFT JOIN tbl_warehouse ON tbl_warehouse.wh_id = tbl_wh_data.wh_id
									WHERE tbl_warehouse.dist_id <> '' AND
												tbl_wh_data.report_month='$sel_month' AND
												tbl_wh_data.report_year='$sel_year' 
												$qStrPro $qStrStk $qStrProv
									GROUP BY tbl_warehouse.dist_id,tbl_warehouse.stkid
									ORDER BY getProvName(prov_id),getDistrictNameByID(dist_id),getSktName(stkid)";
				  $rsItem2 = mysql_query($queryItem2) or die(mysql_error());
				  ...
				  if($numItem2>0) { 
					...
					while($rowItem2 = mysql_fetch_array($rsItem2)) { 
					   ...
					   if($old2!=$rowItem2['dist_id']){ $districtName = getDistrictName($rowItem2['dist_id']);}
					   
					   if($old3!=$rowItem2['prov_id']){ $provinceName = getProvinceName1($rowItem2['prov_id']);}else {$provinceName = "";}
					...
					//   print "SELECT REPgetConsumptionAVG('FSPD','$sel_month','$sel_year','".$sel_item."','".$rowItem2['stkid']."','".$rowItem2['prov_id']."','".$rowItem2['dist_id']."') FROM DUAL<br>";
						...
						$MOS2 = number_format(mysql_result($query,0,0),1);
...
<script>
	...
	function doInitGrid(){
		...
		mygrid.setHeader("<div style='text-align:center; font-size:14px; font-weight:bold; font-family:Helvetica'><?php echo "PPIU Report For Stakeholder = $stkName Province/Region = $provinceName  And Product = $prodName (".date('F',mktime(0,0,0,$sel_month)).' '.$sel_year.")";?></div>,#cspan,#cspan,#cspan,#cspan");
		mygrid.attachHeader("<span title='Provincial Project Implementation Unit Name'>PPIU's Name</span>,<span title='Stakeholder Name'>Stakeholder</span>,<span title='Average Monthly Consumption'>AMC</span>,<span title='Total'>Total</span>,<span title='Month of Scale'>MOS</span>");
