...
if($_POST['stk_sel'] != 2)
{
		...
		if ($_POST['stk_sel'] == 'all'){
			...
			$where = " Office.lvl = '".$_REQUEST['wh_type']."' AND tbl_warehouse.stkid = '".$_POST['stk_sel']."'";
		}
...
Inner Join stakeholder AS Office ON Office.stkid = tbl_warehouse.stkofficeid WHERE $where AND tbl_wh_data.report_year = '".$_POST['year_sel']."' AND tbl_wh_data.report_month = '".$i."' AND tbl_wh_data.item_id = '".$itemName."'");
					$row1 = mysql_fetch_array($getTotal);
					$monthSum = number_format($row1['total']);
					$xmlstore .="\t\t<cell>$monthSum</cell>\n";
				}
				...
				for ($i=1; $i<=12; $i++){
					$getTotal = mysql_query("SELECT sum(tbl_wh_data.wh_cbl_a) as total FROM tbl_warehouse
Inner Join tbl_wh_data ON tbl_wh_data.wh_id = tbl_warehouse.wh_id
Inner Join stakeholder AS Office ON Office.stkid = tbl_warehouse.stkofficeid  WHERE $where AND tbl_wh_data.report_year = '".$_POST['year_sel']."' AND tbl_wh_data.report_month = '".$i."' AND tbl_wh_data.item_id = '".$itemName."'");
					$row1 = mysql_fetch_array($getTotal);
					$monthSum = number_format($row1['total']);
				...
				for ($i=1; $i<=12; $i++){
					$getTotal = mysql_query("SELECT sum(tbl_wh_data.wh_received) as total FROM tbl_warehouse
Inner Join tbl_wh_data ON tbl_wh_data.wh_id = tbl_warehouse.wh_id
Inner Join stakeholder AS Office ON Office.stkid = tbl_warehouse.stkofficeid  WHERE $where AND tbl_wh_data.report_year = '".$_POST['year_sel']."' AND tbl_wh_data.report_month = '".$i."' AND tbl_wh_data.item_id = '".$itemName."'");
					$row1 = mysql_fetch_array($getTotal);
					$monthSum = number_format($row1['total']);
...
Inner Join stakeholder AS Office ON Office.stkid = tbl_warehouse.stkofficeid
 WHERE Office.lvl < 3 AND tbl_wh_data.report_year = '".$sel_year."' AND tbl_wh_data.report_month = '".$i."' AND tbl_wh_data.item_id = '".$itemName."'");
				$row1 = mysql_fetch_array($getTotal);
				$monthSum = number_format($row1['total']);
				$xmlstore .="\t\t<cell>$monthSum</cell>\n";
			}
