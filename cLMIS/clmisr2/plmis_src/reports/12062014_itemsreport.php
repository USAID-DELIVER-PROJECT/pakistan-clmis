<?php
	...
	//print "GET";
		...
		if(isset($_GET['item_id']) && !empty($_GET['item_id'])){
			...
			$qStrPro1 = " AND tbl_wh_data.item_id = '".$_GET['item_id']."'";
		}	
...
*/		
		...
		if(isset($_POST['prod_sel']) && !empty($_POST['prod_sel']) && $_POST['prod_sel']!='all'){
			...
			$qStrPro1 = " AND tbl_wh_data.item_id = '".$_POST['prod_sel']."'";
		}
	...
	else 
	{
		...
		$qStrPro1 = " AND tbl_wh_data.item_id = 'IT-001'";
		$sel_prov = '1';	
 ...
 $xmlstore2 .="</rows>\n";
				...
				$queryItem = "SELECT DISTINCT Office.stkname AS firstCol,
				  ...
				  if($numItem>0) { 
					...
					// get all districts
					$qryDist = mysql_query("SELECT DISTINCT
											...
											and tbl_wh_data.report_month='$sel_month' and tbl_wh_data.report_year='$sel_year'
											 $qStrPro1 $qStrStk $qStrProv");
					while ( $dist = mysql_fetch_array($qryDist) )
					{
						$distArr[] = $dist['dist_id'];
					}
				  ...
				  if($numItem2>0) { 
					...
					while($rowItem2 = mysql_fetch_array($rsItem2)) { 
						...
						if ( in_array($rowItem2['dist_id'], $distArr) )
						{
							$store = number_format($store);
							$MOS = number_format($MOS, 2);
							$field = number_format($field);
							$MOS1 = number_format($MOS1, 2);
							$totalAmt = number_format($totalAmt);
							$MOS2 = number_format($MOS2, 2);
						}
						else
						{
							$store = 'UNK';
						...
						/*$store = ($store == 0) ? 'UNK' : number_format($store);
						$MOS = ($MOS == 0) ? 'UNK' : number_format($MOS, 2);
						$field = ($field == 0) ? 'UNK' : number_format($field);
						$MOS1 = ($MOS1 == 0) ? 'UNK' : number_format($MOS1, 2);
						$totalAmt = ($totalAmt == 0) ? 'UNK' : number_format($totalAmt);
						$MOS2 = ($MOS2 == 0) ? 'UNK' : number_format($MOS2, 2);*/
