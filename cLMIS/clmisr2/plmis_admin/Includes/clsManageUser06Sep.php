...
try{
		...
		if ($this->m_whrec_id=='') $this->m_whrec_id='NULL';
		if ($this->m_stkid=='') $this->m_stkid=0;
		if ($this->m_dist_id=='') $this->m_dist_id='NULL';
		if ($this->m_prov_id=='') $this->m_prov_id='0';
		if ($this->m_usrlogin_id=='') $this->m_usrlogin_id='NULL';	
		if ($this->m_full_name=='') $this->m_full_name='NULL';
		if ($this->m_email_id=='') $this->m_email_id=0;
		if ($this->m_phone_no=='') $this->m_phone_no='NULL';
		if ($this->m_sysusr_photo=='') $this->m_sysusr_photo='NULL';
		if ($this->m_sysusr_dept=='') $this->m_sysusr_dept='NULL';
		if ($this->m_sysusr_deg=='') $this->m_sysusr_deg='NULL';
		if ($this->m_sysusr_type=='') $this->m_sysusr_type='NULL';
		...
		sysusr_name,sysusr_pwd,sysusr_status,sysusr_email,sysusr_ph,sysusr_cell,sysusr_addr,sysusr_dept,sysusr_deg,sysusr_type,sysusr_photo) 
		VALUES(NOW(),".($this->GetMaxUserId()+1).",".$this->m_stkid.",".$this->m_whrec_id.",".$this->m_prov_id.",'".$this->m_usrlogin_id."','".$this->m_full_name."','".$this->m_sysusr_pwd."','Active','".$this->m_email_id."','".$this->m_phone_no."','".$this->m_fax_no."','".$this->m_address."','".$this->m_sysusr_dept."','".$this->m_sysusr_deg."','".$this->m_sysusr_type."','".$this->m_sysusr_photo."')";
		/*print $strSql; 
		exit;*/
  ...
  exit;
	...
	function EditUser(){
		...
		if ($this->m_whrec_id !='') $strSql .=$m_whrec_id;
		
		$usrlogin_id=",usrlogin_id='".$this->m_usrlogin_id."'";
		if ($this->m_usrlogin_id!='') $strSql .=$usrlogin_id;
		
		$sysusr_pwd=",sysusr_pwd='".$this->m_sysusr_pwd."'";
		if ($this->m_sysusr_pwd!='') $strSql .=$sysusr_pwd;
		
		$full_name=",sysusr_name='".$this->m_full_name."'";
		if ($this->m_full_name!='') $strSql .=$full_name;
		
		$phone_no=",sysusr_ph='".$this->m_phone_no."'";
		if ($this->m_phone_no!='') $strSql .=$phone_no;
		
		$email_id=",sysusr_email='".$this->m_email_id."'";
		if ($this->m_email_id!='') $strSql .=$email_id;
		...
		if ($this->m_sysusr_dept!='') $strSql .=$sysusr_dept;
		
		$sysusr_deg=",sysusr_deg='".$this->m_sysusr_deg."'";
		if ($this->m_sysusr_deg!='') $strSql .=$sysusr_deg;
		
		$sysusr_type=",sysusr_type='".$this->m_sysusr_type."'";
		if ($this->m_sysusr_type!='') $strSql .=$sysusr_type;
		
		$m_sysusr_photo=",sysusr_photo='".$this->m_sysusr_photo."'";
		if ($this->m_sysusr_photo!='') $strSql .=$m_sysusr_photo;
