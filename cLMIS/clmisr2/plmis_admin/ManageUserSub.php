...
$rsStakeholders = $objstk->GetStakeholdersByUserId($stk);

$objloc->LocLvl=2;
$rsloc=$objloc->GetLocationsById($prov);

include("xml/xml_genaration_usersubadmin.php");
//Put XML file name and mysql table name simultaniously
writeXML('usersubadmin.xml',$stk,$prov);
...
if($strDo == "Delete" )
{
	...
	header("location:ManageUserSub.php");
}
...
function editFunction(val){
	window.location="ManageUserSub.php?Do=Edit&Id="+val;
}
function delFunction(val){
	if (confirm("Are you sure you want to delete the record?")){
		window.location="ManageUserSub.php?Do=Delete&Id="+val;
		}	
}
...
function doInitGrid(){
	...
	mygrid.init();
	mygrid.loadXML("xml/usersubadmin.xml");
}
...
<tr>
	<td>
		...
		<form method="post" action="ManageUserSubAction.php" name="manageuser" id="manageuser" enctype='multipart/form-data'>
