...
if (isset($_REQUEST['lmisName']) && isset($_REQUEST['ccemName']))
{
	$qry = "UPDATE import_refrigerators
			...
			WHERE
				import_refrigerators.ft_manu_name = '".$_REQUEST['ccemName']."'";
	mysql_query($qry);
	exit;
}

// Update Makes
mysql_query("UPDATE import_refrigerators,
				ccm_makes
				SET import_refrigerators.MakeID = ccm_makes.pk_id
				WHERE
					import_refrigerators.ft_manu_name = ccm_makes.ccm_make_name");
?>
...
<body>
    ...
    <table id="myTable" width="700">
		...
		$ccmQry = "SELECT DISTINCT
					...
					import_refrigerators.ft_manu_name
				FROM
        ...
        ?>
            <tr>
                ...
                <td><?php echo $row['ft_manu_name'];?></td>
                <td>
                    <select name="ccmMap" id="<?php echo $row['ft_manu_name'];?>" onchange="updateFunc(this.value, '<?php echo $row['ft_manu_name']?>')">
...
<?php
echo '<a href="asset_sub_type.php">Next</a>';