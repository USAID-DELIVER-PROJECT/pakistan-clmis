    ...
    $.inlineEdit = function (urls, options) {
        ...
        $('.editableSingle').click(function () {
            ...
            if (!editableFields || $('.editField').length < editableFields) {
                ...
                if ($('.editFieldFirst').length == 0) {
                    ...
                    $(this).html('<div class="editFieldWrapper"><input type="text" value="' + value + '" class="form-control editField editFieldFirst span2" /></div>');
                    if (!simpleMode) {
        ...
        function addSaveControllers(callback)
        {
            if ($('.editFieldWrapper:last').parent().hasClass('removable')) {
                $('.editFieldWrapper:last').append('<div class="editFieldSaveControllers"><button class="btn btn-primary input-sm">Save</button>' +
                        ', <a href="javascript:;" class="editFieldRemove">Remove</a> or ' +
                        '<a href="javascript:;" class="editFieldCancel"><button class="btn btn-warning input-sm">Cancel</button></a></div>');
            } else {
                $('.editFieldWrapper:last').append('<div class="editFieldSaveControllers"><button class="btn btn-primary input-sm">Save</button> or ' +
                        '<a href="javascript:;" class="editFieldCancel"><button class="btn btn-warning input-sm">Cancel</button></a></div>');
            }
